<template>
  <div class="relative h-full flex justify-center items-end gap-4">
    ...
    >
      ...
      >
        <div
          class="absolute top-0 left-0 w-full h-full rounded-br-full bg-light-50 dark:bg-[#293462]"
        ></div>
      </div>
      <div
        class="absolute bottom-0 right-0 h-4 aspect-square bg-blue-600 transform translate-x-full"
      >
        <div
          class="absolute top-0 left-0 w-full h-full rounded-bl-full bg-light-50 dark:bg-[#293462]"
        ></div>
      </div>
    </div>
    ...
    >
      <div
        class="relative z-1 px-4 h-full flex justify-center items-center gap-1 text-lg cursor-pointer"
        :class="
          currentFirstRoute.name === route.name ? 'text-white' : 'text-blue-600'
        "
        v-if="route"
