<template>
  <div class="w-full h-full bg-light-200 flex flex-col">
    <div class="w-full h-16 flex justify-between items-center px-4">
      <h1 class="text-lg">修改政策兑换资金项目</h1>
...
const formData = reactive({
  policy_uuid: '', // 政策uuid
  entp_scale: null as number | null, // 企业规模
  label_uuid: '', // 企业类型标签
  entp_name: '', // 企业名字
  depart: null as number | null, // 部门
  amount: '', // 金额
  pay_date: null as string | null // 发放时间
});

const route = useRoute();
onMounted(async () => {
  const infoList = await getPolicyFunds({ page: 1, page_size: 99999 });
  const info = infoList.items.find((item) => item.uuid === route.params.uuid);
  console.log(infoList, route.params.uuid, info);
  if (info) {
    formData.policy_uuid = info.policy_uuid;
    formData.entp_scale = info.entp_scale;
    formData.label_uuid = info.label_uuid;
    formData.entp_name = info.entp_name;
    formData.depart = info.depart;
    formData.amount = info.amount;
    formData.pay_date = info.pay_date;
  }
});
...
async function submit() {
  ...
  try {
    ...
    const res = await editPolicyFunds(route.params.uuid as string, formData);
    console.log(res);
